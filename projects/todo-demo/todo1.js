...
Todo.prototype.init = function(){
	...
	input.addEventListener('keydown', function(e){
		...
		// 输入框者内容为空，直接返回
		if(this.value.trim() === '') return; //trim去两端空格

		_todo.add(this.value);

		this.value = '';   //添加完成后清空，还原
	}, false);
...
Todo.prototype.add = function(text){
	...
	//2为todo实例添加item元素
	var newItem = new TodoItem(id, text, this);  //this就是把Todo自己传进去
...
Todo.prototype.refresh = function(){   //让数字更新
...
function TodoItem(id, text, app){
	this.ref = null;    //看见Todoitem实例后就找到对应的DOM对象，所以定义一个this.ref,先写成null，因为实例化的时候还不知道是谁
	this.app = app;     //app代指Todoitem的父对象。
	this.id = id;
	this.text = text;
	this.isActive = true;  //新添加的就是未完成的
